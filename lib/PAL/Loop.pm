...
PAL::Loop - High-performance Linux event framework using io_uring
...
PAL::Loop is a high-performance event framework for Linux that leverages
...
sub io {
    ...
    my $watcher = PAL::Loop::Watcher->new(
        id => $self->{next_id}++,
        type => 'io',
        loop => $self,
        fh => $fh,
...
All watchers returned by PAL::Loop methods are objects with the
following methods:
        ...
        cb => sub {
            ...
            $loop->io(
                ...
                cb => sub {
                    ...
                    print $client "Hello from PAL::Loop!\n";
                    close $client;
                }
            );
        }
...
PAL::Loop leverages io_uring for exceptional performance:
...
PAL::Loop automatically detects available features at runtime:

    use PAL::Loop;
    
    print "inotify available\n" if $PAL::Loop::INOTIFY_AVAILABLE;
    print "fanotify available\n" if $PAL::Loop::FANOTIFY_AVAILABLE;
